    ...
    if (lastResult?.is_last_question) {
      ...
      try {
        ...
        if (report.status === "completed") {
          navigate(`/interview/report/${session.sessionId}`, { state: { report } });
        } else {
          // Rare: still processing — retry once after a delay
          await new Promise((r) => setTimeout(r, 3000));
          const report2 = await getInterviewReport(session.sessionId);
          navigate(`/interview/report/${session.sessionId}`, { state: { report: report2 } });
        }
      } catch (err) {
